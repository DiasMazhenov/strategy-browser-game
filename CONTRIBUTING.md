...
> ⚠️ **PR в этом репозитории не используются (правило с 1.0.126).**
> `main` обновляется **прямым пушем** (`git push origin main`), без pull request.
> Входящий PR закрывается автоматически — workflow `ci/no-pr-workflow.yml`.
> Ниже раздел «Правила для PR» читайте как «правила для кода»: они обязательны
> независимо от того, как изменения попадают в `main`.
