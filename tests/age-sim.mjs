...
console.log('\n=== 3. Итоги закрываются (золотой век — только после посвящения) ===');
{
  // п.22: золотой век ДЕРЖИТ свиток открытым, пока хан не выбрал посвящение, —
  // иначе бонус эпохи проскакивался бы кнопкой «дальше». Старое поведение
  // («кнопка закрывает экран») осталось для обычных эпох.
  game.closeAgeReport();
  game.pushHud();
  ok(hud.ageReport !== null, 'золотой век не отпускает, пока не выбрано посвящение');
  game.chooseDedication('kosh');
  game.pushHud();
  ok(hud.ageReport === null, 'после посвящения свиток закрывается');
  ok(game.eraDedication === 'kosh', 'посвящение записалось (Ұлы көш)');
}
