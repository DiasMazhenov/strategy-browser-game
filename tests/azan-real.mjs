...
console.log('\n=== 5. Поведение в игре ===');
{
  ...
  // по умолчанию режим выключен — азан всё равно по игровым суткам
  game.pushHud();
  ok(game.settings.realAzan === false, 'по умолчанию азан по игровым суткам');
  // расписание в HUD есть ВСЕГДА (справочно: «до намаза столько-то»), но пока
  // режим выключен — флаг on=false: механика реального времени не работает.
  ok(hud.realAzan && hud.realAzan.on === false,
    'режим выключен — в HUD on:false (расписание показывается справочно)');
